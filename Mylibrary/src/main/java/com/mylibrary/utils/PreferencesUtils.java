...
public class PreferencesUtils {
    ...
    private static final String filename = "giftday";

    static {
        try {
            c = MyApplication.getInstance().createPackageContext(filename, Context.CONTEXT_IGNORE_SECURITY);
...
//            }else{
            return c.getSharedPreferences(filename + "_preferences", Context.MODE_WORLD_READABLE | Context.MODE_WORLD_WRITEABLE);
//            }
        } else {
    ...
    public static boolean getBoolean(String name, boolean defaultVaule) {
        return getSP().getBoolean(name, defaultVaule);
    }
